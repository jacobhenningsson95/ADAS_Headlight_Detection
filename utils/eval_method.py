...
def gaussian_eval(gt_points, dt_points, scalar=32, value_only=False):
    ...
    if dt_num_car == gt_num_car:
        cases_counting = np.array([1, 0, 0])  # number of predicted cars = ground true cars
    elif dt_num_car > gt_num_car:
        cases_counting = np.array([0, 1, 0])  # number of predicted cars > ground true cars
    else:
        cases_counting = np.array([0, 0, 1])  # number of predicted cars < ground true cars
    ...
    if not value_only:
        ...
        return eval_result, cases_counting


def gaussian_method(batch_tags_results, keypoints, batch_size, scalars=None):
    """
    batch_tags_results: batch_size predict keypoints
    keypoints: ground truth keypoints
    batch_size: batch_size
    scalar: (scalar=𝝈^𝟐) covariance matrix of 2D Gaussian
    """
    if scalars is None:
        scalars = [16*16, 32*32, 64*64, 128*128]

    keypoints = keypoints.tolist()
    # for each img
    mean_eval_score_batch_dic = {}
    cases_counting_batch = np.array([0, 0, 0])
    for scalar in scalars:
        eval_results = 0
        for i in range(batch_size):
            gt_points = keypoints[i][0]
            dt_points = batch_tags_results[i]
            eval_value, cases_counting = gaussian_eval(gt_points, dt_points, scalar=scalar, value_only=True)
            eval_results += eval_value
            if scalar == scalars[0]:  # only need to compute once
                cases_counting_batch += cases_counting

        mean_eval_score_batch = eval_results/batch_size
        mean_eval_score_batch_dic[scalar] = mean_eval_score_batch
    return mean_eval_score_batch_dic, cases_counting_batch
