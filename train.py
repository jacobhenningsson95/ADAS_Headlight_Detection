...
def train(config, train_loader, model, criterion, optimizer, device, plot_dict=None):
    ...
    train_loss = []
    for img, keypoints_ref, keypoints, img_name, heatmaps in tqdm(train_loader):
        ...
        num_loss = len(config['loss'])
        # related to original code: pose.py line 116
        losses = {i[0]: result[-num_loss + idx] * i[1] for idx, i in enumerate(config['loss'])}
        loss = 0
        for i in losses:
...
def validate(config, val_loader, model, criterion, device):
    ...
    with torch.no_grad():
        ...
        for img, keypoints_ref, keypoints, img_name, heatmaps in tqdm(val_loader):
            img = img.to(device)
            keypoints_ref = keypoints_ref.to(device)
            heatmaps = heatmaps.to(device)
            with autocast(enabled=config['autocast']):
            ...
            #Sclae the different losses according to the configuration file.
            losses = {i[0]: result[-num_loss + idx] * i[1] for idx, i in enumerate(config['loss'])}
            loss = 0
            for i in losses:
                loss = loss + torch.mean(losses[i])
